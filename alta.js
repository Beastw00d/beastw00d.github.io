(function() {
    ...
    function lineup() {
      ...
      function findOtherLineups() {
        var valid = [];
        var selectedPlayers = [vm.line1A, vm.line1B, vm.line2A, vm.line2B, vm.line3A,
             vm.line3B, vm.line4A, vm.line4B, vm.line5A, vm.line5B]
        ...
        for (var x = 0; x < results.length; x++) {        
          ...
          line5B = guys[9];
            ...
            valid.push({
              line_1: line1A.name + ' - ' + line1B.name + ': ' + sum(line1A.points, line1B.points),
              line_2: line2A.name + ' - ' + line2B.name + ': ' + sum(line2A.points, line2B.points),
              line_3: line3A.name + ' - ' + line3B.name + ': ' + sum(line3A.points, line3B.points),
              line_4: line4A.name + ' - ' + line4B.name + ': ' + sum(line4A.points, line4B.points),
              line_5: line5A.name + ' - ' + line5B.name + ': ' + sum(line5A.points, line5B.points)
            });                 
        }
      ...
      function permutator(inputArr) {
        ...
        function permute(arr, memo) {
          var cur, memo = memo || [];
          if(!validate(memo)) return results;
          for (var i = 0; i < arr.length; i++) {
            cur = arr.splice(i, 1); 
            
            if (arr.length === 0) {
              var result = memo.concat(cur);
              if(!validate(result)) return results;
